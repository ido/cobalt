        ...
        for b in self._blocks.values():
            ...
            for block in b._relatives:
                ...
                if block.cleanup_pending:
                    ...
                    break
            if cleaning:
                if self.bridge_partition_cache[block.name].state == pybgsched.Block.Free:
                ...
                for b in self._blocks.values():
                    ...
                    if b.cleanup_pending:
                        if b.used_by:
                            ...
                            parts.append(b)
                            for part in parts:
                                if part.block_type != 'pseudoblock' and self.bridge_partition_cache[part.name].getStatus() != pybgsched.Block.Free:
                                    busy.append(part.name)
                                elif part.block_type == 'pseudoblock' and self.bridge_partition_cache[part.subblock_parent].getStatus() != pybgsched.Block.Free:
                                    busy.append(part.name)
                            if len(busy) > 0:
                    ...
                    if b.state == "busy":
                        ...
                        #block_jobs = _get_jobs_on_block(b.subblock_parent)
                        #if (b.block_type != "pseudoblock" and  #check to see if this is needed for any block
                                #b.size > 128 and #FIXME: make this settable in the cobalt.conf
                                #len(block_jobs) == 1 and
                                #block_jobs[0].getComputeNodesUsed() == b.size):
