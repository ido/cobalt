...
class Reservation (Data):
    ...
    def update (self, spec):
        if spec.has_key("users"):
            qm = ComponentProxy(COMP_QUEUE_MANAGER)
            try:
...
class JobDict(ForeignDataDict):
    ...
    __function__ = ComponentProxy(COMP_QUEUE_MANAGER).get_jobs
...
class QueueDict(ForeignDataDict):
    ...
    __function__ = ComponentProxy(COMP_QUEUE_MANAGER).get_queues
...
class ClusterSched (Component):
    
    implementation = "csched"
        ...
        for i in range(len(reservations)):
            for j in range(i+1, len(reservations)):
                ...
                if res1.overlaps(res2.start, res2.duration - 0.00001):
                    ...
                    results = ComponentProxy(COMP_SYSTEM).get_partitions(
    ...
    def _run_reservation_jobs (self, reservations_cache):
        ...
        for cur_res in reservations_cache.itervalues():
            ...
            try:
                best_partition_dict = ComponentProxy(COMP_SYSTEM).find_job_location(job_location_args, [])
            except:
    ...
    def _start_job(self, job, partition_list):
        cqm = ComponentProxy(COMP_QUEUE_MANAGER)
...
#        try:
#            equiv = ComponentProxy(COMP_SYSTEM).find_queue_equivalence_classes(res_info, [q.name for q in active_queues + spruce_queues])
#        except:
#            self.logger.error("failed to connect to system component")
#            return
        ...
        for eq_class in equiv:
            ...
            try:
                best_partition_dict = ComponentProxy(COMP_SYSTEM).find_job_location(job_location_args, end_times)
            except:
