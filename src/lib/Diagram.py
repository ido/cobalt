...
def mod_reserv_dict(reserv_dict, part_list, npart_dict, dep_info):
    ...
    for ((user, name, start, duration, end), part_list) in reserv_dict.iteritems():
        ...
        top_part = [npart_dict[curr_part] for curr_part in part_list if npart_dict[curr_part].get('size') == max_size][0].get('name')        
        # saves the following notation for each reservation into a new dictionary
        if len([curr_part for curr_part in part_list if curr_part in dep_info[top_part][1]]) == len(dep_info[top_part][1]):
...
def gen_part_hits(reserv_dict, npart_dict):
    ...
    for part_one in reserv_dict[reserv_one]:
        for part_two in reserv_dict[reserv_two]:
            if part_one == part_two: # compares regardless of partition size
                ...
                curr_part_conflicts_all.append(part_one) # adds curr shared partition to list of all conflicting partitions for the two reservs
                part_conflicts[(reserv_one, reserv_two)] = curr_part_conflicts_all # assigns updated list as value to the pair of time-conflicting reservs 
            else: # partitions != same
                pass
    if len(curr_part_conflicts_32) == 0: # temp list has no conflicting 32-node partitions
        del part_conflicts_times[(reserv_one, reserv_two)] # removes pairs of reservs with conflicting times, but no conflicting partitions; no real conflict
    else: # temp list has conflicting 32-node partitions 
        part_conflicts_times[(reserv_one, reserv_two)].append(curr_part_conflicts_32) # appends list of sorted 32-node conflicting partitions 
    return;

def detect_conflicts(reserv_dict, npart_dict): 
        ...
        for list_item in reserv_list: # chooses first reserv
            ...
            for next_item in reserv_list[iterator:]: # choose second reserv
                ...
                if list_item[2] >= next_item[4] or list_item[4] <= next_item[2]:
                    ...
                    # appends correct conflict zones' start and end times; use ret_short_str() on epoch times for debugging
                    ...
                    detect_shared_parts(list_item, next_item, reserv_dict) # time conflict exists, then check for partition conflicts
...
def prnt_plain_conflicts(reserv_dict): 
    ...
    if (len(part_conflicts) == 0): # checks if entries exist in the global conflicts dict
        ...
        header = [("User1", "Reservation1", "StartTime1", "EndTime1", "User2", "Reservation2", "StartTime2", "EndTime2")] 
        ...
        for pair in part_conflicts.keys():
            ...
            format_conflicts.insert(next_index, (pair[0][0], pair[0][1], ret_short_str(pair[0][2]), ret_short_str(pair[0][4]), pair[1][0], pair[1][1], ret_short_str(pair[1][2]), ret_short_str(pair[1][4]))) 
            next_index += 1 # advances index to placeholder of next reserv pair
        Cobalt.Util.print_tabular(header + format_conflicts) # prints information in organized fashion
        print("\n")
    return;

def prnt_verbose_conflicts(reserv_dict):
    ...
    if (len(part_conflicts) == 0): # checks if entries exist in the global conflicts dictionary
        ...
        header = [("User1", "Reservation1", "StartTime1", "EndTime1", "User2", "Reservation2", "StartTime2", "EndTime2")] 
        for pair in part_conflicts.keys():
            # pair[0][*] = attributes of first reserv; pair[1][*] = attributes of second reserv; prints information in organized fashion
            Cobalt.Util.print_tabular(header + [(pair[0][0], pair[0][1], ret_short_str(pair[0][2]), ret_short_str(pair[0][4]), pair[1][0], pair[1][1], ret_short_str(pair[1][2]), ret_short_str(pair[1][4]))]) 
...
def gen_image(reserv_dict, args):
    ...
    canvas.text((canvas_x * 0.5 - canvas.textsize(image_title)[0] * 0.5, canvas_y * 0.03), fill = (0, 0, 0), text = image_title, font = def_font) 
    ...
    # draws statistic information centered in terms of canvas's width; 'textsize' returns tuple (x, y) of last string character 
    ...
    canvas.text((canvas_x * 0.5 - canvas.textsize(stat_info)[0] * 0.5, canvas_y * 0.05), fill = (255, 0, 0), text = stat_info, font = def_font) 
    ...
    mesh_start_pixel = canvas.textsize(part_label)[0] + 20 # 20 pixels slack space; 10 pixels on each side of the partition label 
    ...
    rect_width = mesh_width / num_days # width of a single rectangle in the mesh; changes based on the duration of all reservs measured in days = num_days); MUST MAKE rect_width dynamic                                                   
    rect_height = mesh_width * 0.015 # height of a single rectangle in the mesh; 1.5% the mesh width (do not change percentage)
 
    mesh_x = mesh_start_pixel # starting x-coordinate of the first rectangle in the mesh; 
    mesh_y = canvas_y * 0.08  # starting y-coordinate of the first rectangle in the mesh; 8% of the mesh width (do not change percentage)
    ...
    label_names.reverse() #'R001_J216-32,'...'R000_J104-32', 'R000_J102-32'; reversed so the labels of the 32-node partitions from the top midplain printed first


    labels_x = canvas_x * 0.01 # starting x-coordinate of the y-axis labels; 1% of the canvas width (do not change percentage)
    labels_y = mesh_y          # starting y-coordinate of the y-axis labels; 8% of the canvas width (increases progressively)   
    ...
    for curr_reserv in reserv_dict.keys():
        ...
        start_pixel = (particular_start_time / time_per_pixel) + label_width # calculate reservation start pixel based on mesh
        end_pixel = (particular_end_time / time_per_pixel) + label_width     # calculate reservation end pixel based on mesh
    ...
    for curr_reserv in reserv_dict.keys():      
        ...
        reserv_xy[curr_reserv].append(int(math.floor(part_y[reserv_part_hits[curr_reserv][-1]][0]))) # y1 [-1] = top node group by name (bottom border) e.g. 117 (upper in the graph, hence smaller depth), [0] = y1 (first of 2 values in tuple)
        reserv_xy[curr_reserv].append(int(math.floor(part_y[reserv_part_hits[curr_reserv][0]][1]))) # y2   [0] = first node group by name (upper border) e.g. 111 (lower in the graph, hence bigger depth), [1] = y2 (second of 2 values in tuple)

    # CREATES THE RESERVATION COLORS

    reserv_colors = {} # key = reservation name, val= color (tuple form)
    colors = [(255, 131, 250), (139, 123, 139), (255, 215, 0), (165, 42, 42), (124, 252, 0), (255, 64, 64), (30, 144, 255), (255, 255, 0), (205, 127, 50), (153, 204, 50), (138, 43, 226), (255, 0, 255), (127, 255, 212), (139, 115, 85), (219, 219, 112),  (255, 193, 193), (0, 0, 238), (244, 164, 96), (95, 159, 159), (165, 42, 42), (193, 205, 193), (139, 105, 20), (70, 130, 180), (171, 130, 255), (240, 128, 128)]
    colors_copy = colors

    # DRAWS THE RESERVATIONS = LAYER 1

    for curr_reserv in reserv_xy.keys():
        color = colors_copy[0] # always selects the head of the list
        reserv_colors[curr_reserv[1]] = color # save the color for later so the legend can retrieve based on reservation name only
        colors_copy.remove(color) # no duplicate selection of the same color; hence the copy of the array as to keep the original intact
        canvas.rectangle([(reserv_xy[curr_reserv][0], reserv_xy[curr_reserv][2]), (reserv_xy[curr_reserv][1], reserv_xy[curr_reserv][3])], fill = color) # same color fill and outline so I don't have to graph it line by line; no outline so it does not add to pixel size of rectangles
    ...
    for conflict in part_conflicts_times.keys():
        ...
        conflict_start_pixel_y = int(math.floor(part_y[temp_node_names[-1]][0])) # y1 [-1] = top node group by name (bottom border) e.g. 117 (upper in the graph, hence smaller depth), [0] = y1 (first of 2 values in tuple)
        conflict_end_pixel_y = int(math.floor(part_y[temp_node_names[0]][1])) # y2 [0] = first node group by name (upper border) e.g. 111 (lower in the graph, hence bigger depth), [1] = y2 (second of 2 values in tuple) 
        #canvas.rectangle([(conflict_start_pixel_x, conflict_start_pixel_y), (conflict_end_pixel_x, conflict_end_pixel_y)], fill = (0, 0, 0)) #use black rectangles to map conflicts
        #print (conflict, conflict_start_pixel_x, conflict_end_pixel_x, conflict_start_pixel_y, conflict_end_pixel_y)     
        ...
        copy_conflict_start_pixel_x = conflict_start_pixel_x # fresh copy as to not directly modify the require variable
        copy_conflict_start_pixel_y = conflict_start_pixel_y # fresh copy as to not directly modify the require variable   
        ...
        for column in xrange(0, duration_y): # modify this; column does not play a role right now
            for row in xrange(0, duration_x): # modify this; row does not play a role right now
                if (copy_conflict_start_pixel_y >= conflict_end_pixel_y) and (copy_conflict_start_pixel_x >= conflict_end_pixel_x):      
                    break              
                if (copy_conflict_start_pixel_x >= conflict_end_pixel_x) and (copy_conflict_start_pixel_y < conflict_end_pixel_y):
                ...
                if (copy_conflict_start_pixel_y >= conflict_end_pixel_y) and (copy_conflict_start_pixel_x < conflict_end_pixel_x): # end of the y-coordinates
                ...
                #back slash of 'X'
                canvas.line([(copy_conflict_start_pixel_x, copy_conflict_start_pixel_y), (copy_conflict_start_pixel_x + step, copy_conflict_start_pixel_y + step)], fill = cross_hatch_color) 
                #forward slash of 'X'                   
                canvas.line([(copy_conflict_start_pixel_x, copy_conflict_start_pixel_y + step), (copy_conflict_start_pixel_x + step, copy_conflict_start_pixel_y)], fill = cross_hatch_color)
    ...
    for label_num in xrange(0, len(label_names)): # currently labels exist for 32 node groups of 32 nodes each, 0 to 32 = 32
        # find proper ratio for labels_y + ? * rect_height
        canvas.text((labels_x, labels_y + 0.25 * rect_height), fill = (0, 0, 0), text = label_names[label_num], font = def_font) # + 0.25 * rect_height for y so text centered in terms of 1st adjacent mesh rectangle
        labels_y = labels_y + rect_height # increase the depth or y coordinate by the size of an individual rectangle

    # DRAWS THE MESH = LAYER 3B

    num_columns = num_days + 1 # 1 to num_days + 1 = num_days columns
    num_rows = len(label_names) + 2 # 1 through 34 (32 node groups + 2) = 33 rows = 32 for node groups and 1 for ruler
    for day in xrange(1, num_columns): 
        for node_group in xrange(1, num_rows): 
            if node_group == num_rows - 1: # if last, row is yellow because it is the ruler 
                canvas.rectangle([(mesh_x, mesh_y), (mesh_x + rect_width , mesh_y + rect_height)], fill = (255, 246, 143), outline = (0, 0, 0)) 
            else: # else, row has no fill and is transparent
                canvas.rectangle([(mesh_x, mesh_y), (mesh_x + rect_width , mesh_y + rect_height)], outline = (0, 0, 0)) 
            mesh_y = mesh_y + rect_height # rectangles in rows forming cols, one above another, for rows must have growing height
        ...
        canvas.line([(mesh_x + 0.5 * rect_width, mesh_y - 10), (mesh_x + 0.5 * rect_width, mesh_y + 10)], fill = tick_color)
        # generates minor tick marks
        canvas.line([(mesh_x + 0.25 * rect_width, mesh_y - 5), (mesh_x + 0.25 * rect_width, mesh_y + 5)], fill = tick_color)
        canvas.line([(mesh_x + 0.75 * rect_width, mesh_y - 5), (mesh_x + 0.75 * rect_width, mesh_y + 5)], fill = tick_color)
        # generates miniscule tick marks
        canvas.line([(mesh_x + 0.125 * rect_width, mesh_y - 3), (mesh_x + 0.125 * rect_width, mesh_y + 3)], fill = tick_color)
        canvas.line([(mesh_x + 0.375 * rect_width, mesh_y - 3), (mesh_x + 0.375 * rect_width, mesh_y + 3)], fill = tick_color)
        canvas.line([(mesh_x + 0.625 * rect_width, mesh_y - 3), (mesh_x + 0.625 * rect_width, mesh_y + 3)], fill = tick_color)
        canvas.line([(mesh_x + 0.875 * rect_width, mesh_y - 3), (mesh_x + 0.875 * rect_width, mesh_y + 3)], fill = tick_color)
        ...
        canvas.text((mesh_x - canvas.textsize(year_r)[0] * 0.5, mesh_y + 20), fill = (0, 0, 255), text = year_r, font = def_font)
        canvas.text((mesh_x - canvas.textsize(month_r)[0] * 0.5, mesh_y + 30), fill = (0, 0, 255), text = month_r, font = def_font)
        canvas.text((mesh_x - canvas.textsize(day_r)[0] * 0.5, mesh_y + 40), fill = (0, 0, 255), text = day_r, font = def_font)
        canvas.text((mesh_x - canvas.textsize(time_r)[0] * 0.5, mesh_y + 50), fill = (0, 0, 255), text = time_r, font = def_font)
        ...
        if day == num_columns - 1:
            ...
            canvas.text((mesh_x_new - canvas.textsize(year_r)[0] * 0.5, mesh_y_new + 20), fill = (0, 0, 255), text = year_r, font = def_font)
            canvas.text((mesh_x_new - canvas.textsize(month_r)[0] * 0.5, mesh_y_new + 30), fill = (0, 0, 255), text = month_r, font = def_font)
            canvas.text((mesh_x_new - canvas.textsize(day_r)[0] * 0.5, mesh_y_new + 40), fill = (0, 0, 255), text = day_r, font = def_font)
            canvas.text((mesh_x_new - canvas.textsize(time_r)[0] * 0.5, mesh_y_new + 50), fill = (0, 0, 255), text = time_r, font = def_font)

        mesh_x = mesh_x + rect_width # rectangles' cols forming rows, one next to another
        mesh_y = mesh_y_copy # mesh_y = canvas * 0.08 for cols must have consistent height; resets to original value for next iteration 
       
    # DRAWS MIDPLANE SEPARATORS 

    # generates top line
    canvas.line([(labels_x, mesh_y_copy + rect_height * 0), (mesh_x_copy, mesh_y_copy + rect_height * 0)], fill = (255, 0, 0))
    # generates middle line
    canvas.line([(labels_x, mesh_y_copy + rect_height * (num_rows - 2) / 2), (mesh_x_copy, mesh_y_copy + rect_height * (num_rows - 2)/ 2)], fill = (255, 0, 0))
    # generates bottom line
    canvas.line([(labels_x, mesh_y_copy + rect_height * (num_rows - 2)), (mesh_x_copy, mesh_y_copy + rect_height * (num_rows - 2))], fill = (255, 0, 0))
    
    # DRAWS RESERVATIONS LEGEND

    legend_x = mesh_x_copy # unmodified mesh_x, x-coordinate for the legend title text
    legend_y = num_rows * rect_height + mesh_y_copy + 60 # 33 rows * width of each rectangle + start point of mesh + 50 points to jump of time labels
    ...
    legend_y += 20 # so 'RESERVATIONS LEGEND' does not overlap with the first rectangle
    ...
    for curr_reserv in reserv_dict.keys():
        ...
        canvas.rectangle([(legend_x, legend_y), (legend_x + legend_square_side, legend_y + legend_square_side)], fill = reserv_colors[curr_reserv[1]], outline = (0, 0, 0)) # draws color rectangle
        canvas.text((legend_x + legend_square_side + 10, legend_y + 0.25 * legend_square_side), fill = (0, 0, 0), text = reserv_text) #draws reservation text
    ...
    if options.graph[3] == "y":
        ...
        conflicts_y = num_rows * rect_height + mesh_y_copy + 60 #33 rows * width of each rectangle + start point of mesh + 50 points to jump of time labels
        ...
        conflicts_y += 20 # space between entries and 'DETECTED CONFLICTS'
        ...
        if len(part_conflicts) > 0:
            for curr_conflict in part_conflicts.keys():
                conf_text = curr_conflict[0][0] + "/" + curr_conflict[0][1] + " | " + curr_conflict[1][0] + "/" + curr_conflict[1][1]
            ...
            canvas.text((conflicts_x, conflicts_y + 0.25 * legend_square_side), fill = (0, 0, 0), text = "no conflicts exist")

    # EXPORTS THE IMAGE
    image_name = options.graph[0] + "_" + user_s + "@" + host_s + "_" + time.strftime("%Y%m%d_%H%M%S", time.localtime(time.time())) + "_" + str(canvas_x) + "x" + str(canvas_y) 
    ...
    os.execl("/soft/apps/tools/ImageMagick/bin/display", "display", "./" + image_name + "." + options.graph[1], "&") # views the generated image
...
if __name__ == '__main__':
    ...
    depinfo = Cobalt.Util.buildRackTopology(partitions) # builds topology; key = partition, value = ((parents), (children))
    
    mod_reserv_dict(reservations, partitions, npart, depinfo) #modifies the 'reservation' dictionary to a different format
    gen_part_hits(reservations, npart) # builds data structure of reservations and their lists of 32-node partition 
    detect_conflicts(reservations, npart) # stores any detected conflicts to the data structure 'part_conflicts_times'

    parser = optparse.OptionParser(usage="%prog [-options]", version="\n'%prog' written by " + orig_author  + " (denkov@mcs.anl.gov)\nversion: " 
                                   + ver_num + " date: " + ver_date + " modified by: " + last_mod + "\n\nACKNOWLEDGEMENTs:"
    ...
    parser.add_option("-p", "--plain", action="store_true", dest="confplain", help="print a plain summary of conflicting reservations pairs") 
    parser.add_option("-v", "--verbose", action="store_true", dest="confverbose", help="print a verbose summary of conflicting reservations pairs")
    parser.add_option("-g", "--graph", nargs=4, dest="graph", help="generates an image representing the current reservations in the system")
    parser.add_option("-s", "--showres", action="store_true", dest="showres", help="call the 'showres' command with the -l option selected")
    ...
    if options.confplain and len(args) == 0:   # -p and no args
        prnt_plain_conflicts(reservations) 
    ...
    if options.confverbose and len(args) == 0:   # -v and no args
        prnt_verbose_conflicts(reservations)
