...
my $name		= 'jobmanager-cobalt';
my $manager_type	= 'cobalt';
my $cmd;
my $non_cluster		= 0;
my $cpu_per_node	= 1;
...
my $metadata =
    new Grid::GPT::Setup(package_name => "globus_gram_job_manager_setup_cobalt");
...
print `./find-cobalt-tools $non_cluster --with-cpu-per-node=$cpu_per_node --with-remote-shell=$remote_shell --cache-file=/dev/null`;
if($? != 0)
{
    print STDERR "Warning: Error locating Cobalt commands!\n";
    exit 0;
}

# Create service
$cmd = "$libexecdir/globus-job-manager-service -add -m cobalt -s \"$name\"";
...
open(VALIDATION_FILE, ">$ENV{GLOBUS_LOCATION}/share/globus_gram_job_manager/cobalt.rvf");    
